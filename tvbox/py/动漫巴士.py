...
class Vod:
    def __init__(self):
        ...
        # self.config()
        # self.log(f'{os.path.dirname(os.path.abspath(__file__))}')
        # self.log(f"{os.listdir('/data/user/0/com.fongmi.android.tv/')}")
        self.read_file()
    ...
    def config(self):
        ...
        self.log(f"缓存路径：{cache_dir}")
        # 外部存储
        external_dir = Python.getPlatform().getApplication().getExternalFilesDir(None).getAbsolutePath()
        self.log(f"外部存储路径：{external_dir}")
        # 内部存储目录
        internal_dir = Python.getPlatform().getApplication().getFilesDir().getAbsolutePath()
        self.log(f"内部存储目录: {internal_dir}")

        # 获取公共路径
        # public_directory = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
        public_directory = Environment.getExternalStorageDirectory().getAbsolutePath()
        self.log(f"获取公共路径: {public_directory}")

    def read_file(self):
        # os.listdir('../../files')
        try:
            path = os.path.dirname(os.path.abspath(__file__)).split('/cache')[0] + '/files/TV/.quarkcookie'
            if os.path.exists(path):
                with open(path, 'r' ,encoding="utf-8") as f:
                    a = f.read()
                    self.log(f'读取文件成功： {a}')
        except Exception as e:
            self.log(f'读取文件成功失败：{e}')
...
if __name__ == '__main__':
    """
    缓存路径：/data/user/0/com.fongmi.android.tv.ok/cache
    外部存储路径：/storage/emulated/0/Android/data/com.fongmi.android.tv.ok/files
    内部存储目录: /data/user/0/com.fongmi.android.tv.ok/files
    获取公共路径: /storage/emulated/0
    """
    pass
