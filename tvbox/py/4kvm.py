...
class Spider(Spider):
  ...
  def _extract_episodes(self, doc) -> list:
    episodes = []
    seenIds = set()
    selectors = [".grid.grid-cols-6.gap-2 a[href^='/play/']", "a.episode-link"]
  ...
  def detailContent(self, ids: list) -> str:
    ...
    episodes = self._extract_episodes(doc)
