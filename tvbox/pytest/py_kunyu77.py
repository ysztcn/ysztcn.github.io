...
class Spider(Spider):
    ...
    def homeContent(self, filter):
        ...
        for cKey in jData.keys():
            classes.append({"type_name": jData[cKey][0]["cat"], "type_id": cKey})
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        suffix = "".join(params)
        url = (
            "http://api.kunyu77.com/api.php/provide/searchFilter?pagesize=24{0}".format(
                suffix
            )
        )
        rsp = self.fetch(url, headers=self.header)
    ...
    def detailContent(self, array):
        tid = array[0]
        url = "http://api.kunyu77.com/api.php/provide/videoDetail?devid=453CA5D864457C7DB4D0EAA93DE96E66&package=com.sevenVideo.app.android&version=1.8.7&ids={0}".format(
            tid
        )
        rsp = self.fetch(url, headers=self.header)
        jo = json.loads(rsp.text)
        ...
        listUrl = "http://api.kunyu77.com/api.php/provide/videoPlaylist?devid=453CA5D864457C7DB4D0EAA93DE96E66&package=com.sevenVideo.app.android&version=1.8.7&ids={0}".format(
            tid
        )
        ...
        for ep in episodes:
            ...
            for playurl in playurls:
                ...
                if source not in playMap.keys():
                    playMap[source] = []
                playMap[source].append(
                    playurl["title"].strip() + "$" + playurl["playurl"]
                )
    ...
    def searchContent(self, key, quick):
        url = (
            "http://api.kunyu77.com/api.php/provide/searchVideo?searchName={0}".format(
                key
            )
        )
        rsp = self.fetch(url, headers=self.header)
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        result = {"parse": 0, "jx": 0, "playUrl": "", "url": id, "header": ""}
        if flag in vipFlags:
            result["parse"] = 1
            result["jx"] = 1
        return result
