...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def homeContent(self, filter):
        ...
        vodList = root.select("div.ewave-vodlist__box div.lazyload")
        videos = []
        for vod in vodList:
            name = vod["title"]
    ...
    def detailContent(self, array):
        tid = array[0]
        url = self.host + "/voddetail/{0}.html".format(tid)
        rsp = self.fetch(url, headers=self.header)
        root = BeautifulSoup(rsp.text, "lxml")
        title = root.select("h1.title")[0].text
        pic = root.select("div.picture>img")[0]["data-original"]
        vod = {
            "vod_id": tid,
            "vod_name": title,
            "vod_pic": pic,
        ...
        for e in root.select("span.text-muted"):
            txt = "".join(e.strings)
            if txt[0:3] == "类型：":
                vod["type_name"] = e.next_sibling
            elif txt[0:3] == "年份：":
                vod["vod_year"] = e.next_sibling
            elif txt[0:3] == "地区：":
                vod["vod_area"] = e.next_sibling
            elif txt[0:3] == "主演：":
                vod["vod_actor"] = e.next_sibling
            elif txt[0:3] == "导演：":
                vod["vod_director"] = e.next_sibling
            elif txt[0:3] == "简介：":
                vod["vod_content"] = e.next_sibling

        playFrom = []

        vodHeader = root.select("ul.nav-tabs li a")
        for v in vodHeader:
            playFrom.append(v.text.split("（")[0])
        vod_play_from = "$$$".join(playFrom)

        playList = []
        vodList = root.select("div.tab-content ul")
        for vl in vodList:
            vodItems = []
    ...
    def searchContent(self, key, quick):
        ...
        root = BeautifulSoup(rsp.text, "lxml")
        vodList = root.select("a.lazyload")
        videos = []
        for vod in vodList:
            name = vod["title"]
            if name.find(key) < 0:
            ...
            sid = vod["href"]
            sid = self.regStr(sid, "/voddetail/(\\S+).html")
            videos.append({"vod_id": sid, "vod_name": name, "vod_pic": pic, "vod_remarks": ""})
        result = {"list": videos}
        return result
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        result = {}
        url = self.host + "/vodplay/{0}.html".format(id)
        ...
        rsp = self.fetch(
            self.host
            + "/static/js/playerconfig.js?t={0}".format(datetime.date.today().strftime("%y%M%d"))
        )
        pcJson = json.loads(self.regStr(rsp.text, "player_list*=(\{.+?\}),MacPlayerConfig"))
        targetUrl = pcJson[mJson["from"]]["parse"] + mJson["url"]
        result["parse"] = 1
        # try:
        #     rsp = self.fetch(targetUrl)
        #     ctxt = self.regStr(rsp.text,"\"url\":.*\"(.+?)\"")
        #     iv = self.regStr(rsp.text,"le_token.*=.*\"(.+?)\";")
        #     url = self.CBC_Decrypt(ctxt, "S8J4G60C2B408472", iv)
        #     if url!="":
        #         targetUrl = url
        #         result["parse"] = 0
        # except:
