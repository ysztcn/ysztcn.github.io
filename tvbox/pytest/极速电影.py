...
class Spider(Spider):
    ...
    def categoryContent(self, cid, pg, filter, ext):
        ...
        if page == '1':
            ...
            url = f'{xurl}/index.php/vod/show/id/{cid}/page/{str(page)}/year/{NdType}.html'
    ...
    def detailContent(self, ids):
        ...
        tiaozhuan = '1'
        if tiaozhuan == '1':
            did = self.extract_middle_text(res, '<div class="play-btn clearfix">', '<!-- end 播放器 -->', 1, 'href="(.*?)"')
            if 'http' not in did:
                did = xurl + did
                res1 = requests.get(url=did, headers=headerx)
                res1.encoding = "utf-8"
        ...
        xianlu = self.extract_middle_text(res, '<ul class="nav nav-tabs  pull-right">','</ul>',2, 'data-toggle="tab">(.*?)</a>')
    ...
    def searchContentPage(self, key, quick, page):
        ...
        if not page:
            page = '1'
        if page == '1':
            url = f'{xurl}/index.php/vod/search.html?wd={key}&submit='
        ...
        soups = doc.find_all('ul', class_="stui-vodlist__media col-pd clearfix")
