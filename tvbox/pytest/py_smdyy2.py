...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def homeContent(self, filter):
        ...
        vodList = root.xpath(
            "//ul[contains(@class,'stui-vodlist')][position() != last()]//a[contains(@class,'lazyload')]"
        )
        videos = []
        for vod in vodList:
            ...
            mark = vod.xpath("./span[contains(@class,'pic-text')]/text()")[0]
    ...
    def detailContent(self, array):
        ...
        pic = root.xpath("//div[contains(@class,'stui-content__thumb')]//img/@data-original")[0]
        vod = {
            "vod_id": tid,
            "vod_name": title,
            "vod_pic": pic,
        ...
        for e in root.xpath("//p[@class='data']/text()"):
            txt = e
        ...
        vod["vod_content"] = root.xpath("//span[@class='detail-content']")[0].text
        playFrom = []
        vodHeader = root.xpath("//div[@class='stui-pannel clearfix']//h3/text()")
        for v in vodHeader:
            playFrom.append(v)
        vod_play_from = "$$$".join(playFrom)

        playList = []
        vodList = root.xpath("//div[@class='stui-pannel clearfix']//ul")
        for vl in vodList:
            vodItems = []
