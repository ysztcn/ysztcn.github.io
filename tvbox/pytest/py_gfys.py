...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def homeContent(self, filter):
        ...
        vodList = root.xpath("//div[contains(@class,'lazyload')]")
        videos = []
        for vod in vodList:
    ...
    def detailContent(self, array):
        ...
        rsp = self.fetch(url, headers=self.header)
        root = self.html(rsp.text)
        title = root.xpath("//h1/span/text()")[0]
        pic = root.xpath("//div[contains(@class,'picture')]/img/@data-original")[0]
        vod = {
            "vod_id": tid,
            "vod_name": title,
            "vod_pic": pic,
        ...
        for e in root.xpath("//span[contains(@class,'text-muted')]"):
            txt = e.xpath("./text()")[0]
            v = e.xpath("./following-sibling::text()[1]")
            if v == []:
                v = [""]
            if txt[0:3] == "类型：":
                vod["type_name"] = v[0]
                ...
                vod["vod_director"] = v[0]
            elif txt[0:3] == "简介：":
                vod["vod_content"] = v[0]

        playFrom = []

        vodHeader = root.xpath("//ul[contains(@class,'nav-tabs')]/li/a")
        for v in vodHeader:
            playFrom.append(v.xpath("./text()")[0].split("（")[0])
        vod_play_from = "$$$".join(playFrom)

        playList = []
        vodList = root.xpath("//div[contains(@class,'tab-content')]//ul")
        for vl in vodList:
            vodItems = []
    ...
    def searchContent(self, key, quick):
        ...
        root = self.html(rsp.text)
        vodList = root.xpath("//a[contains(@class,'lazyload')]")
        videos = []
        for vod in vodList:
            name = vod.xpath("./@title")[0]
            if name.find(key) < 0:
