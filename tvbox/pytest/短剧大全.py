...
class Spider(Spider):
    ...
    def categoryContent(self, cid, pg, filter, ext):
        ...
        if page == '1':
            url = f'{xurl}/index.php/vod/type/class/{cid}/id/41/page/{str(page)}/year/{NdType}.html'
        ...
        try:
            ...
            for soup in soups:
                ...
                for vod in vods:
                    ...
                    names = vod.find('a', class_="thumb")
                    name = names['title']

                    ids = vod.find('a', class_="thumb")
                    id = ids['href']

                    pics = vod.find('a', class_="thumb")
                    pic = pics.find('img')['data-original']

                    remarks = vod.find('span', class_="state")
    ...
    def detailContent(self, ids):
        ...
        if name not in content:
            ...
            bofang = self.extract_middle_text(res, '<ul class="episodes-list clearfix">', '</ul>', 3, 'href="(.*?)" rel=".*?">(.*?)</a>')

        xianlu = self.extract_middle_text(res, '<ul class="nav nav-tabs swiper-wrapper">','</ul>',2, 'title=".*?">(.*?)</a>')
